...
function OutfitCard({
  ...
  const visualize = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      setImgError('Could not generate image. Try again.')
    } finally {
  ...
  return (
    ...
    >
      ...
      <div className="px-5 pb-4">
        <h2 className="text-xl font-extralight tracking-tight text-white leading-tight">{outfit.outfit_name}</h2>
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          <span className="text-zinc-500 text-[11px] italic">{outfit.vibe}</span>
          <span className="text-zinc-800 text-[9px]">·</span>
          <span className="text-[9px] uppercase tracking-[0.2em] text-zinc-600 border border-zinc-900 rounded-full px-2 py-0.5">{outfit.style}</span>
        </div>
      </div>
        ...
        {(Object.keys(PIECE_LABELS) as (keyof OutfitPieces)[]).map((key) => {
          ...
          return (
            <div key={key} className="flex gap-3 items-start">
              <span className="text-[9px] uppercase tracking-[0.2em] text-zinc-700 w-12 flex-shrink-0 pt-1">{PIECE_LABELS[key]}</span>
              <div className="flex-1 flex items-start justify-between gap-2 min-w-0">
                <span className="text-[12px] text-zinc-200 leading-snug flex-1">{piece.item}</span>
                {shop && (
      ...
      {(outfit.how_to_wear || []).length > 0 && (
        <>
          ...
          <div className="px-5 py-4">
            <p className="text-[9px] uppercase tracking-[0.25em] text-zinc-600 mb-3">How to wear it</p>
            <div className="space-y-2">
              {outfit.how_to_wear.map((step, i) => (
                <div key={i} className="flex gap-3 items-start">
                  ...
                  <span className="text-[12px] text-zinc-400 leading-snug">{step}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="px-5 pb-2">
        <button onClick={() => setShowDetails(v => !v)} className="w-full text-left flex items-center justify-between group py-2">
          <span className="text-[9px] uppercase tracking-[0.25em] text-zinc-700 group-hover:text-zinc-400 transition-colors">Why it works for you</span>
        ...
        {showDetails && (
          <div className="pb-3 space-y-3 animate-in fade-in duration-200">
            <p className="text-[12px] text-zinc-400 leading-relaxed">{outfit.why_it_works}</p>
            {outfit.styling_tip && (
              <div className="border-l-2 border-zinc-800 pl-3">
                <p className="text-[9px] uppercase tracking-[0.2em] text-zinc-600 mb-1">Pro tip</p>
                <p className="text-[12px] text-zinc-400 leading-relaxed">{outfit.styling_tip}</p>
              </div>
            )}
          </div>
        )}
      ...
      <div className="px-5 pb-5 pt-1 space-y-2.5">
        ...
        >
          {visualizing ? (
            ...
            generatedImage ? 'Regenerate Image' : 'Visualize Outfit'
          )}
        </button>
...
export default function AIStylist() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setError('Could not reach the stylist. Check your connection.')
    } finally {
      setGenerating(false)
    }
  }
        ...
        <div className="max-w-sm mx-auto px-5 pt-20 pb-8 space-y-10" style={{ paddingBottom: 'max(2rem, env(safe-area-inset-bottom))' }}>
          ...
          {profile && (
            <div className="flex flex-wrap gap-2">
              {profile.skin_tone && <span className="text-[9px] uppercase tracking-widest text-zinc-600 border border-zinc-900 rounded-full px-3 py-1.5">{profile.skin_tone} skin</span>}
          ...
          {/* ── Occasion ── */}
          <section>
            <p className="text-[10px] uppercase tracking-[0.25em] text-zinc-500 mb-4">{isAr ? 'المناسبة' : 'Occasion'}</p>
            <div className="flex flex-wrap gap-2">
              {OCCASIONS.map(o => (
          ...
          {/* ── Vibe ── */}
          <section>
            <p className="text-[10px] uppercase tracking-[0.25em] text-zinc-500 mb-4">{isAr ? 'الأجواء' : 'Your Vibe'}</p>
            <div className="flex flex-wrap gap-2">
              {VIBES.map(v => (
          ...
          {/* ── Season ── */}
          <section>
            <p className="text-[10px] uppercase tracking-[0.25em] text-zinc-500 mb-4">{isAr ? 'الموسم' : 'Season'}</p>
          ...
          <button onClick={generate} disabled={generating}
            ...
            {generating ? (
              <span className="flex items-center justify-center gap-3">
                ...
                {isAr ? 'جارٍ التنسيق...' : 'Curating your looks...'}
              </span>
            ) : (isAr ? 'توليد إطلالات' : 'Generate Looks')}
          </button>
          ...
          {outfits.length > 0 && !generating && (
            <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <p className="text-center">
                <ShinyText
                  text={`${outfits.length} looks curated for you`}
              ...
              {outfits.map((outfit, i) => (
                <OutfitCard key={i} outfit={outfit} index={i} onSave={saveOutfit} saved={savedNames.has(outfit.outfit_name)} gender={(profile?.gender as Gender) || 'male'} profile={profile} />
              ))}
