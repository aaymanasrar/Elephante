const STOP_WORDS = new Set([
  ...
  'me','suggest','something','what','kind','of','do','you','have',
])
...
const BODY_SHAPE_FITS = {
  slim: { good: ['layered','oversized','relaxed','wide','straight','baggy','puffer','chunky'], bad: ['slim fit','skinny','tapered','fitted','tight'] },
  athletic: { good: ['straight','relaxed','tapered','slim fit','structured'], bad: ['oversized','baggy','boxy'] },
  average: { good: ['straight','regular','relaxed','classic'], bad: [] },
  stocky: { good: ['straight','dark','monochrome','vertical','slim fit','tapered'], bad: ['oversized','baggy','wide leg','loose','bulky'] },
  heavy: { good: ['dark','straight','monochrome','vertical','relaxed','structured'], bad: ['tight','fitted','slim fit','skinny','bright'] },
} as const

function buildBlob(item: Outfit): string {
  return [
    item?.style,
    item?.style_category,
    item?.occasion,
    ...
    item?.top_wear, item?.bottom_wear,
    item?.shoes, item?.accessories, item?.outerwear,
    item?.material_notes, item?.outfit_details,
    item?.brand, item?.occasions,
...
export function smartMatch(query: string, item: Outfit): boolean {
...
export function scoreOutfit(outfit: Outfit, skinTone: string, palettes: string[], bodyShape?: string): number {
  let score = 0
  const scheme = (outfit.color_scheme || outfit.aesthetic || '').toLowerCase()
  const blob = [
    outfit.top_wear, outfit.bottom_wear, outfit.shoes,
    outfit.outfit_details, outfit.style_category, outfit.style,
  ].filter(Boolean).join(' ').toLowerCase()

  if (palettes.includes('dark') && (scheme.includes('dark') || scheme.includes('moody'))) score += 3
  if (palettes.includes('neutral') && scheme.includes('neutral')) score += 3
  if (palettes.includes('pastel') && (scheme.includes('pastel') || scheme.includes('soft'))) score += 3
  if (palettes.includes('colorful') && (scheme.includes('color') || scheme.includes('vibrant'))) score += 3

  if ((skinTone === 'dark' || skinTone === 'tan') &&
      (scheme.includes('vibrant') || scheme.includes('color') || scheme.includes('dark'))) score += 1
  if ((skinTone === 'light' || skinTone === 'medium') &&
      (scheme.includes('neutral') || scheme.includes('pastel') || scheme.includes('dark'))) score += 1

  if (bodyShape && bodyShape in BODY_SHAPE_FITS) {
    const { good, bad } = BODY_SHAPE_FITS[bodyShape as keyof typeof BODY_SHAPE_FITS]
    for (const keyword of good) {
      if (blob.includes(keyword)) score += 2
    }
    for (const keyword of bad) {
      if (blob.includes(keyword)) score -= 3
    }
  }
